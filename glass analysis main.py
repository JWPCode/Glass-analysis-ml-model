import math
import numpy as np
import random
import plotly.graph_objects as go
import networkx as nx


class Connection:
    def __init__(self, connectedNeuron):
        self.connectedNeuron = connectedNeuron
        self.weight = np.random.normal()
        self.dWeight = 0.0


class Neuron:
    eta = 0.001
    alpha = 0.01

    def __init__(self, layer):
        self.dendrons = []
        self.error = 0.0
        self.gradient = 0.0
        self.output = 0.0
        if layer is None:
            pass
        else:
            for neuron in layer:
                con = Connection(neuron)
                self.dendrons.append(con)

    def addError(self, err):
        self.error = self.error + err

    def sigmoid(self, x):
        return 1 / (1 + math.exp(-x * 1.0))

    def dSigmoid(self, x):
        return x * (1.0 - x)

    def setError(self, err):
        self.error = err

    def setOutput(self, output):
        self.output = output

    def getOutput(self):
        return self.output

    def feedForword(self):
        sumOutput = 0
        if len(self.dendrons) == 0:
            return
        for dendron in self.dendrons:
            sumOutput = sumOutput + float(dendron.connectedNeuron.getOutput()) * float(dendron.weight)
        self.output = self.sigmoid(sumOutput)

    def backPropagate(self):
        self.gradient = self.error * self.dSigmoid(self.output)
        for dendron in self.dendrons:
            dendron.dWeight = Neuron.eta * (
            dendron.connectedNeuron.output * self.gradient) + self.alpha * dendron.dWeight
            dendron.weight = dendron.weight + dendron.dWeight
            dendron.connectedNeuron.addError(dendron.weight * self.gradient)
        self.error = 0


class Network:
    def __init__(self, topology):
        self.layers = []
        for numNeuron in topology:
            layer = []
            for i in range(1,numNeuron):
                if (len(self.layers) == 0):
                    layer.append(Neuron(None))
                else:
                    layer.append(Neuron(self.layers[-1]))
            layer.append(Neuron(None))
            layer[-1].setOutput(1)
            self.layers.append(layer)

    def setInput(self, inputs):
        for i in range(len(inputs)):
            self.layers[0][i].setOutput(inputs[i])

    def feedForword(self):
        for layer in self.layers[1:]:
            for neuron in layer:
                neuron.feedForword()

    def backPropagate(self, target):
        for i in range(len(target)):
            self.layers[-1][i].setError(target[i] - self.layers[-1][i].getOutput())
        for layer in self.layers[::-1]:
            for neuron in layer:
                neuron.backPropagate()

    def getError(self, target):
        err = 0
        for i in range(len(target)):
            e = (target[i] - self.layers[-1][i].getOutput())
            err = err + e ** 2
        err = err / len(target)
        err = math.sqrt(err)
        return err

    def getResults(self):
        output = []
        for neuron in self.layers[-1]:
            output.append(neuron.getOutput())
        output.pop()
        return output

    def getThResults(self):
        output = []
        for neuron in self.layers[-1]:
            o = neuron.getOutput() * 10
            o = round(o)
            #if o >= 0.5:
            #    o = 1
            #else:
            #    o = 0
            
            output.append(o)
        output.pop()
        return output

    def output_network(self):

      node_x = []
      node_y = []
      counterx = 0
      countery = 0
      max_hight = 0
      for layer in self.layers:
          temp =len(layer)
          if temp > max_hight:
              max_hight = temp

      for layer in self.layers:
          countery = max_hight/2 - len(layer)/2
          for node in layer:
              node_x.append(counterx)
              node_y.append(countery)
              countery = countery +1
          counterx = counterx +1

      node_trace = go.Scatter(x=node_x, y=node_y, mode='markers',
          hoverinfo='text',
          marker=dict(
              showscale=True,
              # colorscale options
              #'Greys' | 'YlGnBu' | 'Greens' | 'YlOrRd' | 'Bluered' | 'RdBu' |
              #'Reds' | 'Blues' | 'Picnic' | 'Rainbow' | 'Portland' | 'Jet' |
              #'Hot' | 'Blackbody' | 'Earth' | 'Electric' | 'Viridis' |
              colorscale='YlGnBu',
              reversescale=True,
              color=[],
              size=10,
              colorbar=dict(
                  thickness=15,
                  title=dict(
                    text='Node Connections',
                    side='right'
                  ),
                  xanchor='left',
              ),
              line_width=2))
      

      line_x = []
      line_y = []
      counterx = 0
      countery = 0

      
      #this is the issue here need to fix how the lines are drawn (x should be good, y needs fixing)
      #requiers start and end coordinates of lines
      for i,layer in enumerate(self.layers):
          print(f"Test 1 i = {i}")
          countery = max_hight/2 - len(layer)/2
          for j, node in enumerate(layer):

              if i < len(self.layers)-1:
                  
                  for nnode in self.layers[i+1]:
                      line_x.append(i)
                      line_x.append(i+1)
                      line_x.append(None)

                  for k, nnode in enumerate(self.layers[i+1]):
                      line_y.append(countery+j)
                      countery_next = max_hight/2 - len(self.layers[i+1])/2
                      line_y.append(countery_next + k)
                      line_y.append(None)


          counterx = counterx +1

      line_trace = go.Scatter(
          x=line_x, y=line_y,
          line=dict(width=0.5, color='#888'),
          hoverinfo='none',
          mode='lines')
      
      figure = go.Figure(data=[node_trace, line_trace])

      figure.show()
        

      """
      G = nx.random_geometric_graph(200, 0.125)

      edge_x = []
      edge_y = []
      for edge in G.edges():
          x0, y0 = G.nodes[edge[0]]['pos']
          x1, y1 = G.nodes[edge[1]]['pos']
          edge_x.append(x0)
          edge_x.append(x1)
          edge_x.append(None)
          edge_y.append(y0)
          edge_y.append(y1)
          edge_y.append(None)

      edge_trace = go.Scatter(
          x=edge_x, y=edge_y,
          line=dict(width=0.5, color='#888'),
          hoverinfo='none',
          mode='lines')

      node_x = []
      node_y = []
      for node in G.nodes():
          x, y = G.nodes[node]['pos']
          node_x.append(x)
          node_y.append(y)

      node_trace = go.Scatter(
          x=node_x, y=node_y,
          mode='markers',
          hoverinfo='text',
          marker=dict(
              showscale=True,
              # colorscale options
              #'Greys' | 'YlGnBu' | 'Greens' | 'YlOrRd' | 'Bluered' | 'RdBu' |
              #'Reds' | 'Blues' | 'Picnic' | 'Rainbow' | 'Portland' | 'Jet' |
              #'Hot' | 'Blackbody' | 'Earth' | 'Electric' | 'Viridis' |
              colorscale='YlGnBu',
              reversescale=True,
              color=[],
              size=10,
              colorbar=dict(
                  thickness=15,
                  title=dict(
                    text='Node Connections',
                    side='right'
                  ),
                  xanchor='left',
              ),
              line_width=2))
      
      node_adjacencies = []
      node_text = []
      for node, adjacencies in enumerate(G.adjacency()):
          node_adjacencies.append(len(adjacencies[1]))
          node_text.append('# of connections: '+str(len(adjacencies[1])))

      node_trace.marker.color = node_adjacencies
      node_trace.text = node_text


      fig = go.Figure(data=[edge_trace, node_trace],
             layout=go.Layout(
                title=dict(
                    text="<br>Network graph made with Python",
                    font=dict(
                        size=16
                    )
                ),
                showlegend=False,
                hovermode='closest',
                margin=dict(b=20,l=5,r=5,t=40),
                annotations=[ dict(
                    text="Python code: <a href='https://plotly.com/python/network-graphs/'> https://plotly.com/python/network-graphs/</a>",
                    showarrow=False,
                    xref="paper", yref="paper",
                    x=0.005, y=-0.002 ) ],
                xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
                yaxis=dict(showgrid=False, zeroline=False, showticklabels=False))
                )
      fig.show()
      """

#data stored in the worst way posible 214 values stored with ID's 0 - 213
features = [
  {
    "": 0,
    "RI": 1.52101,
    "Na": 13.64,
    "Mg": 4.49,
    "Al": 1.1,
    "Si": 71.78,
    "K": 0.06,
    "Ca": 8.75,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 1,
    "RI": 1.51761,
    "Na": 13.89,
    "Mg": 3.6,
    "Al": 1.36,
    "Si": 72.73,
    "K": 0.48,
    "Ca": 7.83,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 2,
    "RI": 1.51618,
    "Na": 13.53,
    "Mg": 3.55,
    "Al": 1.54,
    "Si": 72.99,
    "K": 0.39,
    "Ca": 7.78,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 3,
    "RI": 1.51766,
    "Na": 13.21,
    "Mg": 3.69,
    "Al": 1.29,
    "Si": 72.61,
    "K": 0.57,
    "Ca": 8.22,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 4,
    "RI": 1.51742,
    "Na": 13.27,
    "Mg": 3.62,
    "Al": 1.24,
    "Si": 73.08,
    "K": 0.55,
    "Ca": 8.07,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 5,
    "RI": 1.51596,
    "Na": 12.79,
    "Mg": 3.61,
    "Al": 1.62,
    "Si": 72.97,
    "K": 0.64,
    "Ca": 8.07,
    "Ba": 0.0,
    "Fe": 0.26
  },
  {
    "": 6,
    "RI": 1.51743,
    "Na": 13.3,
    "Mg": 3.6,
    "Al": 1.14,
    "Si": 73.09,
    "K": 0.58,
    "Ca": 8.17,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 7,
    "RI": 1.51756,
    "Na": 13.15,
    "Mg": 3.61,
    "Al": 1.05,
    "Si": 73.24,
    "K": 0.57,
    "Ca": 8.24,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 8,
    "RI": 1.51918,
    "Na": 14.04,
    "Mg": 3.58,
    "Al": 1.37,
    "Si": 72.08,
    "K": 0.56,
    "Ca": 8.3,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 9,
    "RI": 1.51755,
    "Na": 13.0,
    "Mg": 3.6,
    "Al": 1.36,
    "Si": 72.99,
    "K": 0.57,
    "Ca": 8.4,
    "Ba": 0.0,
    "Fe": 0.11
  },
  {
    "": 10,
    "RI": 1.51571,
    "Na": 12.72,
    "Mg": 3.46,
    "Al": 1.56,
    "Si": 73.2,
    "K": 0.67,
    "Ca": 8.09,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 11,
    "RI": 1.51763,
    "Na": 12.8,
    "Mg": 3.66,
    "Al": 1.27,
    "Si": 73.01,
    "K": 0.6,
    "Ca": 8.56,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 12,
    "RI": 1.51589,
    "Na": 12.88,
    "Mg": 3.43,
    "Al": 1.4,
    "Si": 73.28,
    "K": 0.69,
    "Ca": 8.05,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 13,
    "RI": 1.51748,
    "Na": 12.86,
    "Mg": 3.56,
    "Al": 1.27,
    "Si": 73.21,
    "K": 0.54,
    "Ca": 8.38,
    "Ba": 0.0,
    "Fe": 0.17
  },
  {
    "": 14,
    "RI": 1.51763,
    "Na": 12.61,
    "Mg": 3.59,
    "Al": 1.31,
    "Si": 73.29,
    "K": 0.58,
    "Ca": 8.5,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 15,
    "RI": 1.51761,
    "Na": 12.81,
    "Mg": 3.54,
    "Al": 1.23,
    "Si": 73.24,
    "K": 0.58,
    "Ca": 8.39,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 16,
    "RI": 1.51784,
    "Na": 12.68,
    "Mg": 3.67,
    "Al": 1.16,
    "Si": 73.11,
    "K": 0.61,
    "Ca": 8.7,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 17,
    "RI": 1.52196,
    "Na": 14.36,
    "Mg": 3.85,
    "Al": 0.89,
    "Si": 71.36,
    "K": 0.15,
    "Ca": 9.15,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 18,
    "RI": 1.51911,
    "Na": 13.9,
    "Mg": 3.73,
    "Al": 1.18,
    "Si": 72.12,
    "K": 0.06,
    "Ca": 8.89,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 19,
    "RI": 1.51735,
    "Na": 13.02,
    "Mg": 3.54,
    "Al": 1.69,
    "Si": 72.73,
    "K": 0.54,
    "Ca": 8.44,
    "Ba": 0.0,
    "Fe": 0.07
  },
  {
    "": 20,
    "RI": 1.5175,
    "Na": 12.82,
    "Mg": 3.55,
    "Al": 1.49,
    "Si": 72.75,
    "K": 0.54,
    "Ca": 8.52,
    "Ba": 0.0,
    "Fe": 0.19
  },
  {
    "": 21,
    "RI": 1.51966,
    "Na": 14.77,
    "Mg": 3.75,
    "Al": 0.29,
    "Si": 72.02,
    "K": 0.03,
    "Ca": 9.0,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 22,
    "RI": 1.51736,
    "Na": 12.78,
    "Mg": 3.62,
    "Al": 1.29,
    "Si": 72.79,
    "K": 0.59,
    "Ca": 8.7,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 23,
    "RI": 1.51751,
    "Na": 12.81,
    "Mg": 3.57,
    "Al": 1.35,
    "Si": 73.02,
    "K": 0.62,
    "Ca": 8.59,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 24,
    "RI": 1.5172,
    "Na": 13.38,
    "Mg": 3.5,
    "Al": 1.15,
    "Si": 72.85,
    "K": 0.5,
    "Ca": 8.43,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 25,
    "RI": 1.51764,
    "Na": 12.98,
    "Mg": 3.54,
    "Al": 1.21,
    "Si": 73.0,
    "K": 0.65,
    "Ca": 8.53,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 26,
    "RI": 1.51793,
    "Na": 13.21,
    "Mg": 3.48,
    "Al": 1.41,
    "Si": 72.64,
    "K": 0.59,
    "Ca": 8.43,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 27,
    "RI": 1.51721,
    "Na": 12.87,
    "Mg": 3.48,
    "Al": 1.33,
    "Si": 73.04,
    "K": 0.56,
    "Ca": 8.43,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 28,
    "RI": 1.51768,
    "Na": 12.56,
    "Mg": 3.52,
    "Al": 1.43,
    "Si": 73.15,
    "K": 0.57,
    "Ca": 8.54,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 29,
    "RI": 1.51784,
    "Na": 13.08,
    "Mg": 3.49,
    "Al": 1.28,
    "Si": 72.86,
    "K": 0.6,
    "Ca": 8.49,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 30,
    "RI": 1.51768,
    "Na": 12.65,
    "Mg": 3.56,
    "Al": 1.3,
    "Si": 73.08,
    "K": 0.61,
    "Ca": 8.69,
    "Ba": 0.0,
    "Fe": 0.14
  },
  {
    "": 31,
    "RI": 1.51747,
    "Na": 12.84,
    "Mg": 3.5,
    "Al": 1.14,
    "Si": 73.27,
    "K": 0.56,
    "Ca": 8.55,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 32,
    "RI": 1.51775,
    "Na": 12.85,
    "Mg": 3.48,
    "Al": 1.23,
    "Si": 72.97,
    "K": 0.61,
    "Ca": 8.56,
    "Ba": 0.09,
    "Fe": 0.22
  },
  {
    "": 33,
    "RI": 1.51753,
    "Na": 12.57,
    "Mg": 3.47,
    "Al": 1.38,
    "Si": 73.39,
    "K": 0.6,
    "Ca": 8.55,
    "Ba": 0.0,
    "Fe": 0.06
  },
  {
    "": 34,
    "RI": 1.51783,
    "Na": 12.69,
    "Mg": 3.54,
    "Al": 1.34,
    "Si": 72.95,
    "K": 0.57,
    "Ca": 8.75,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 35,
    "RI": 1.51567,
    "Na": 13.29,
    "Mg": 3.45,
    "Al": 1.21,
    "Si": 72.74,
    "K": 0.56,
    "Ca": 8.57,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 36,
    "RI": 1.51909,
    "Na": 13.89,
    "Mg": 3.53,
    "Al": 1.32,
    "Si": 71.81,
    "K": 0.51,
    "Ca": 8.78,
    "Ba": 0.11,
    "Fe": 0.0
  },
  {
    "": 37,
    "RI": 1.51797,
    "Na": 12.74,
    "Mg": 3.48,
    "Al": 1.35,
    "Si": 72.96,
    "K": 0.64,
    "Ca": 8.68,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 38,
    "RI": 1.52213,
    "Na": 14.21,
    "Mg": 3.82,
    "Al": 0.47,
    "Si": 71.77,
    "K": 0.11,
    "Ca": 9.57,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 39,
    "RI": 1.52213,
    "Na": 14.21,
    "Mg": 3.82,
    "Al": 0.47,
    "Si": 71.77,
    "K": 0.11,
    "Ca": 9.57,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 40,
    "RI": 1.51793,
    "Na": 12.79,
    "Mg": 3.5,
    "Al": 1.12,
    "Si": 73.03,
    "K": 0.64,
    "Ca": 8.77,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 41,
    "RI": 1.51755,
    "Na": 12.71,
    "Mg": 3.42,
    "Al": 1.2,
    "Si": 73.2,
    "K": 0.59,
    "Ca": 8.64,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 42,
    "RI": 1.51779,
    "Na": 13.21,
    "Mg": 3.39,
    "Al": 1.33,
    "Si": 72.76,
    "K": 0.59,
    "Ca": 8.59,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 43,
    "RI": 1.5221,
    "Na": 13.73,
    "Mg": 3.84,
    "Al": 0.72,
    "Si": 71.76,
    "K": 0.17,
    "Ca": 9.74,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 44,
    "RI": 1.51786,
    "Na": 12.73,
    "Mg": 3.43,
    "Al": 1.19,
    "Si": 72.95,
    "K": 0.62,
    "Ca": 8.76,
    "Ba": 0.0,
    "Fe": 0.3
  },
  {
    "": 45,
    "RI": 1.519,
    "Na": 13.49,
    "Mg": 3.48,
    "Al": 1.35,
    "Si": 71.95,
    "K": 0.55,
    "Ca": 9.0,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 46,
    "RI": 1.51869,
    "Na": 13.19,
    "Mg": 3.37,
    "Al": 1.18,
    "Si": 72.72,
    "K": 0.57,
    "Ca": 8.83,
    "Ba": 0.0,
    "Fe": 0.16
  },
  {
    "": 47,
    "RI": 1.52667,
    "Na": 13.99,
    "Mg": 3.7,
    "Al": 0.71,
    "Si": 71.57,
    "K": 0.02,
    "Ca": 9.82,
    "Ba": 0.0,
    "Fe": 0.1
  },
  {
    "": 48,
    "RI": 1.52223,
    "Na": 13.21,
    "Mg": 3.77,
    "Al": 0.79,
    "Si": 71.99,
    "K": 0.13,
    "Ca": 10.02,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 49,
    "RI": 1.51898,
    "Na": 13.58,
    "Mg": 3.35,
    "Al": 1.23,
    "Si": 72.08,
    "K": 0.59,
    "Ca": 8.91,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 50,
    "RI": 1.5232,
    "Na": 13.72,
    "Mg": 3.72,
    "Al": 0.51,
    "Si": 71.75,
    "K": 0.09,
    "Ca": 10.06,
    "Ba": 0.0,
    "Fe": 0.16
  },
  {
    "": 51,
    "RI": 1.51926,
    "Na": 13.2,
    "Mg": 3.33,
    "Al": 1.28,
    "Si": 72.36,
    "K": 0.6,
    "Ca": 9.14,
    "Ba": 0.0,
    "Fe": 0.11
  },
  {
    "": 52,
    "RI": 1.51808,
    "Na": 13.43,
    "Mg": 2.87,
    "Al": 1.19,
    "Si": 72.84,
    "K": 0.55,
    "Ca": 9.03,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 53,
    "RI": 1.51837,
    "Na": 13.14,
    "Mg": 2.84,
    "Al": 1.28,
    "Si": 72.85,
    "K": 0.55,
    "Ca": 9.07,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 54,
    "RI": 1.51778,
    "Na": 13.21,
    "Mg": 2.81,
    "Al": 1.29,
    "Si": 72.98,
    "K": 0.51,
    "Ca": 9.02,
    "Ba": 0.0,
    "Fe": 0.09
  },
  {
    "": 55,
    "RI": 1.51769,
    "Na": 12.45,
    "Mg": 2.71,
    "Al": 1.29,
    "Si": 73.7,
    "K": 0.56,
    "Ca": 9.06,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 56,
    "RI": 1.51215,
    "Na": 12.99,
    "Mg": 3.47,
    "Al": 1.12,
    "Si": 72.98,
    "K": 0.62,
    "Ca": 8.35,
    "Ba": 0.0,
    "Fe": 0.31
  },
  {
    "": 57,
    "RI": 1.51824,
    "Na": 12.87,
    "Mg": 3.48,
    "Al": 1.29,
    "Si": 72.95,
    "K": 0.6,
    "Ca": 8.43,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 58,
    "RI": 1.51754,
    "Na": 13.48,
    "Mg": 3.74,
    "Al": 1.17,
    "Si": 72.99,
    "K": 0.59,
    "Ca": 8.03,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 59,
    "RI": 1.51754,
    "Na": 13.39,
    "Mg": 3.66,
    "Al": 1.19,
    "Si": 72.79,
    "K": 0.57,
    "Ca": 8.27,
    "Ba": 0.0,
    "Fe": 0.11
  },
  {
    "": 60,
    "RI": 1.51905,
    "Na": 13.6,
    "Mg": 3.62,
    "Al": 1.11,
    "Si": 72.64,
    "K": 0.14,
    "Ca": 8.76,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 61,
    "RI": 1.51977,
    "Na": 13.81,
    "Mg": 3.58,
    "Al": 1.32,
    "Si": 71.72,
    "K": 0.12,
    "Ca": 8.67,
    "Ba": 0.69,
    "Fe": 0.0
  },
  {
    "": 62,
    "RI": 1.52172,
    "Na": 13.51,
    "Mg": 3.86,
    "Al": 0.88,
    "Si": 71.79,
    "K": 0.23,
    "Ca": 9.54,
    "Ba": 0.0,
    "Fe": 0.11
  },
  {
    "": 63,
    "RI": 1.52227,
    "Na": 14.17,
    "Mg": 3.81,
    "Al": 0.78,
    "Si": 71.35,
    "K": 0.0,
    "Ca": 9.69,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 64,
    "RI": 1.52172,
    "Na": 13.48,
    "Mg": 3.74,
    "Al": 0.9,
    "Si": 72.01,
    "K": 0.18,
    "Ca": 9.61,
    "Ba": 0.0,
    "Fe": 0.07
  },
  {
    "": 65,
    "RI": 1.52099,
    "Na": 13.69,
    "Mg": 3.59,
    "Al": 1.12,
    "Si": 71.96,
    "K": 0.09,
    "Ca": 9.4,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 66,
    "RI": 1.52152,
    "Na": 13.05,
    "Mg": 3.65,
    "Al": 0.87,
    "Si": 72.22,
    "K": 0.19,
    "Ca": 9.85,
    "Ba": 0.0,
    "Fe": 0.17
  },
  {
    "": 67,
    "RI": 1.52152,
    "Na": 13.05,
    "Mg": 3.65,
    "Al": 0.87,
    "Si": 72.32,
    "K": 0.19,
    "Ca": 9.85,
    "Ba": 0.0,
    "Fe": 0.17
  },
  {
    "": 68,
    "RI": 1.52152,
    "Na": 13.12,
    "Mg": 3.58,
    "Al": 0.9,
    "Si": 72.2,
    "K": 0.23,
    "Ca": 9.82,
    "Ba": 0.0,
    "Fe": 0.16
  },
  {
    "": 69,
    "RI": 1.523,
    "Na": 13.31,
    "Mg": 3.58,
    "Al": 0.82,
    "Si": 71.99,
    "K": 0.12,
    "Ca": 10.17,
    "Ba": 0.0,
    "Fe": 0.03
  },
  {
    "": 70,
    "RI": 1.51574,
    "Na": 14.86,
    "Mg": 3.67,
    "Al": 1.74,
    "Si": 71.87,
    "K": 0.16,
    "Ca": 7.36,
    "Ba": 0.0,
    "Fe": 0.12
  },
  {
    "": 71,
    "RI": 1.51848,
    "Na": 13.64,
    "Mg": 3.87,
    "Al": 1.27,
    "Si": 71.96,
    "K": 0.54,
    "Ca": 8.32,
    "Ba": 0.0,
    "Fe": 0.32
  },
  {
    "": 72,
    "RI": 1.51593,
    "Na": 13.09,
    "Mg": 3.59,
    "Al": 1.52,
    "Si": 73.1,
    "K": 0.67,
    "Ca": 7.83,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 73,
    "RI": 1.51631,
    "Na": 13.34,
    "Mg": 3.57,
    "Al": 1.57,
    "Si": 72.87,
    "K": 0.61,
    "Ca": 7.89,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 74,
    "RI": 1.51596,
    "Na": 13.02,
    "Mg": 3.56,
    "Al": 1.54,
    "Si": 73.11,
    "K": 0.72,
    "Ca": 7.9,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 75,
    "RI": 1.5159,
    "Na": 13.02,
    "Mg": 3.58,
    "Al": 1.51,
    "Si": 73.12,
    "K": 0.69,
    "Ca": 7.96,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 76,
    "RI": 1.51645,
    "Na": 13.44,
    "Mg": 3.61,
    "Al": 1.54,
    "Si": 72.39,
    "K": 0.66,
    "Ca": 8.03,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 77,
    "RI": 1.51627,
    "Na": 13.0,
    "Mg": 3.58,
    "Al": 1.54,
    "Si": 72.83,
    "K": 0.61,
    "Ca": 8.04,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 78,
    "RI": 1.51613,
    "Na": 13.92,
    "Mg": 3.52,
    "Al": 1.25,
    "Si": 72.88,
    "K": 0.37,
    "Ca": 7.94,
    "Ba": 0.0,
    "Fe": 0.14
  },
  {
    "": 79,
    "RI": 1.5159,
    "Na": 12.82,
    "Mg": 3.52,
    "Al": 1.9,
    "Si": 72.86,
    "K": 0.69,
    "Ca": 7.97,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 80,
    "RI": 1.51592,
    "Na": 12.86,
    "Mg": 3.52,
    "Al": 2.12,
    "Si": 72.66,
    "K": 0.69,
    "Ca": 7.97,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 81,
    "RI": 1.51593,
    "Na": 13.25,
    "Mg": 3.45,
    "Al": 1.43,
    "Si": 73.17,
    "K": 0.61,
    "Ca": 7.86,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 82,
    "RI": 1.51646,
    "Na": 13.41,
    "Mg": 3.55,
    "Al": 1.25,
    "Si": 72.81,
    "K": 0.68,
    "Ca": 8.1,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 83,
    "RI": 1.51594,
    "Na": 13.09,
    "Mg": 3.52,
    "Al": 1.55,
    "Si": 72.87,
    "K": 0.68,
    "Ca": 8.05,
    "Ba": 0.0,
    "Fe": 0.09
  },
  {
    "": 84,
    "RI": 1.51409,
    "Na": 14.25,
    "Mg": 3.09,
    "Al": 2.08,
    "Si": 72.28,
    "K": 1.1,
    "Ca": 7.08,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 85,
    "RI": 1.51625,
    "Na": 13.36,
    "Mg": 3.58,
    "Al": 1.49,
    "Si": 72.72,
    "K": 0.45,
    "Ca": 8.21,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 86,
    "RI": 1.51569,
    "Na": 13.24,
    "Mg": 3.49,
    "Al": 1.47,
    "Si": 73.25,
    "K": 0.38,
    "Ca": 8.03,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 87,
    "RI": 1.51645,
    "Na": 13.4,
    "Mg": 3.49,
    "Al": 1.52,
    "Si": 72.65,
    "K": 0.67,
    "Ca": 8.08,
    "Ba": 0.0,
    "Fe": 0.1
  },
  {
    "": 88,
    "RI": 1.51618,
    "Na": 13.01,
    "Mg": 3.5,
    "Al": 1.48,
    "Si": 72.89,
    "K": 0.6,
    "Ca": 8.12,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 89,
    "RI": 1.5164,
    "Na": 12.55,
    "Mg": 3.48,
    "Al": 1.87,
    "Si": 73.23,
    "K": 0.63,
    "Ca": 8.08,
    "Ba": 0.0,
    "Fe": 0.09
  },
  {
    "": 90,
    "RI": 1.51841,
    "Na": 12.93,
    "Mg": 3.74,
    "Al": 1.11,
    "Si": 72.28,
    "K": 0.64,
    "Ca": 8.96,
    "Ba": 0.0,
    "Fe": 0.22
  },
  {
    "": 91,
    "RI": 1.51605,
    "Na": 12.9,
    "Mg": 3.44,
    "Al": 1.45,
    "Si": 73.06,
    "K": 0.44,
    "Ca": 8.27,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 92,
    "RI": 1.51588,
    "Na": 13.12,
    "Mg": 3.41,
    "Al": 1.58,
    "Si": 73.26,
    "K": 0.07,
    "Ca": 8.39,
    "Ba": 0.0,
    "Fe": 0.19
  },
  {
    "": 93,
    "RI": 1.5159,
    "Na": 13.24,
    "Mg": 3.34,
    "Al": 1.47,
    "Si": 73.1,
    "K": 0.39,
    "Ca": 8.22,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 94,
    "RI": 1.51629,
    "Na": 12.71,
    "Mg": 3.33,
    "Al": 1.49,
    "Si": 73.28,
    "K": 0.67,
    "Ca": 8.24,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 95,
    "RI": 1.5186,
    "Na": 13.36,
    "Mg": 3.43,
    "Al": 1.43,
    "Si": 72.26,
    "K": 0.51,
    "Ca": 8.6,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 96,
    "RI": 1.51841,
    "Na": 13.02,
    "Mg": 3.62,
    "Al": 1.06,
    "Si": 72.34,
    "K": 0.64,
    "Ca": 9.13,
    "Ba": 0.0,
    "Fe": 0.15
  },
  {
    "": 97,
    "RI": 1.51743,
    "Na": 12.2,
    "Mg": 3.25,
    "Al": 1.16,
    "Si": 73.55,
    "K": 0.62,
    "Ca": 8.9,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 98,
    "RI": 1.51689,
    "Na": 12.67,
    "Mg": 2.88,
    "Al": 1.71,
    "Si": 73.21,
    "K": 0.73,
    "Ca": 8.54,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 99,
    "RI": 1.51811,
    "Na": 12.96,
    "Mg": 2.96,
    "Al": 1.43,
    "Si": 72.92,
    "K": 0.6,
    "Ca": 8.79,
    "Ba": 0.14,
    "Fe": 0.0
  },
  {
    "": 100,
    "RI": 1.51655,
    "Na": 12.75,
    "Mg": 2.85,
    "Al": 1.44,
    "Si": 73.27,
    "K": 0.57,
    "Ca": 8.79,
    "Ba": 0.11,
    "Fe": 0.22
  },
  {
    "": 101,
    "RI": 1.5173,
    "Na": 12.35,
    "Mg": 2.72,
    "Al": 1.63,
    "Si": 72.87,
    "K": 0.7,
    "Ca": 9.23,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 102,
    "RI": 1.5182,
    "Na": 12.62,
    "Mg": 2.76,
    "Al": 0.83,
    "Si": 73.81,
    "K": 0.35,
    "Ca": 9.42,
    "Ba": 0.0,
    "Fe": 0.2
  },
  {
    "": 103,
    "RI": 1.52725,
    "Na": 13.8,
    "Mg": 3.15,
    "Al": 0.66,
    "Si": 70.57,
    "K": 0.08,
    "Ca": 11.64,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 104,
    "RI": 1.5241,
    "Na": 13.83,
    "Mg": 2.9,
    "Al": 1.17,
    "Si": 71.15,
    "K": 0.08,
    "Ca": 10.79,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 105,
    "RI": 1.52475,
    "Na": 11.45,
    "Mg": 0.0,
    "Al": 1.88,
    "Si": 72.19,
    "K": 0.81,
    "Ca": 13.24,
    "Ba": 0.0,
    "Fe": 0.34
  },
  {
    "": 106,
    "RI": 1.53125,
    "Na": 10.73,
    "Mg": 0.0,
    "Al": 2.1,
    "Si": 69.81,
    "K": 0.58,
    "Ca": 13.3,
    "Ba": 3.15,
    "Fe": 0.28
  },
  {
    "": 107,
    "RI": 1.53393,
    "Na": 12.3,
    "Mg": 0.0,
    "Al": 1.0,
    "Si": 70.16,
    "K": 0.12,
    "Ca": 16.19,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 108,
    "RI": 1.52222,
    "Na": 14.43,
    "Mg": 0.0,
    "Al": 1.0,
    "Si": 72.67,
    "K": 0.1,
    "Ca": 11.52,
    "Ba": 0.0,
    "Fe": 0.08
  },
  {
    "": 109,
    "RI": 1.51818,
    "Na": 13.72,
    "Mg": 0.0,
    "Al": 0.56,
    "Si": 74.45,
    "K": 0.0,
    "Ca": 10.99,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 110,
    "RI": 1.52664,
    "Na": 11.23,
    "Mg": 0.0,
    "Al": 0.77,
    "Si": 73.21,
    "K": 0.0,
    "Ca": 14.68,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 111,
    "RI": 1.52739,
    "Na": 11.02,
    "Mg": 0.0,
    "Al": 0.75,
    "Si": 73.08,
    "K": 0.0,
    "Ca": 14.96,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 112,
    "RI": 1.52777,
    "Na": 12.64,
    "Mg": 0.0,
    "Al": 0.67,
    "Si": 72.02,
    "K": 0.06,
    "Ca": 14.4,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 113,
    "RI": 1.51892,
    "Na": 13.46,
    "Mg": 3.83,
    "Al": 1.26,
    "Si": 72.55,
    "K": 0.57,
    "Ca": 8.21,
    "Ba": 0.0,
    "Fe": 0.14
  },
  {
    "": 114,
    "RI": 1.51847,
    "Na": 13.1,
    "Mg": 3.97,
    "Al": 1.19,
    "Si": 72.44,
    "K": 0.6,
    "Ca": 8.43,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 115,
    "RI": 1.51846,
    "Na": 13.41,
    "Mg": 3.89,
    "Al": 1.33,
    "Si": 72.38,
    "K": 0.51,
    "Ca": 8.28,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 116,
    "RI": 1.51829,
    "Na": 13.24,
    "Mg": 3.9,
    "Al": 1.41,
    "Si": 72.33,
    "K": 0.55,
    "Ca": 8.31,
    "Ba": 0.0,
    "Fe": 0.1
  },
  {
    "": 117,
    "RI": 1.51708,
    "Na": 13.72,
    "Mg": 3.68,
    "Al": 1.81,
    "Si": 72.06,
    "K": 0.64,
    "Ca": 7.88,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 118,
    "RI": 1.51673,
    "Na": 13.3,
    "Mg": 3.64,
    "Al": 1.53,
    "Si": 72.53,
    "K": 0.65,
    "Ca": 8.03,
    "Ba": 0.0,
    "Fe": 0.29
  },
  {
    "": 119,
    "RI": 1.51652,
    "Na": 13.56,
    "Mg": 3.57,
    "Al": 1.47,
    "Si": 72.45,
    "K": 0.64,
    "Ca": 7.96,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 120,
    "RI": 1.51844,
    "Na": 13.25,
    "Mg": 3.76,
    "Al": 1.32,
    "Si": 72.4,
    "K": 0.58,
    "Ca": 8.42,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 121,
    "RI": 1.51663,
    "Na": 12.93,
    "Mg": 3.54,
    "Al": 1.62,
    "Si": 72.96,
    "K": 0.64,
    "Ca": 8.03,
    "Ba": 0.0,
    "Fe": 0.21
  },
  {
    "": 122,
    "RI": 1.51687,
    "Na": 13.23,
    "Mg": 3.54,
    "Al": 1.48,
    "Si": 72.84,
    "K": 0.56,
    "Ca": 8.1,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 123,
    "RI": 1.51707,
    "Na": 13.48,
    "Mg": 3.48,
    "Al": 1.71,
    "Si": 72.52,
    "K": 0.62,
    "Ca": 7.99,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 124,
    "RI": 1.52177,
    "Na": 13.2,
    "Mg": 3.68,
    "Al": 1.15,
    "Si": 72.75,
    "K": 0.54,
    "Ca": 8.52,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 125,
    "RI": 1.51872,
    "Na": 12.93,
    "Mg": 3.66,
    "Al": 1.56,
    "Si": 72.51,
    "K": 0.58,
    "Ca": 8.55,
    "Ba": 0.0,
    "Fe": 0.12
  },
  {
    "": 126,
    "RI": 1.51667,
    "Na": 12.94,
    "Mg": 3.61,
    "Al": 1.26,
    "Si": 72.75,
    "K": 0.56,
    "Ca": 8.6,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 127,
    "RI": 1.52081,
    "Na": 13.78,
    "Mg": 2.28,
    "Al": 1.43,
    "Si": 71.99,
    "K": 0.49,
    "Ca": 9.85,
    "Ba": 0.0,
    "Fe": 0.17
  },
  {
    "": 128,
    "RI": 1.52068,
    "Na": 13.55,
    "Mg": 2.09,
    "Al": 1.67,
    "Si": 72.18,
    "K": 0.53,
    "Ca": 9.57,
    "Ba": 0.27,
    "Fe": 0.17
  },
  {
    "": 129,
    "RI": 1.5202,
    "Na": 13.98,
    "Mg": 1.35,
    "Al": 1.63,
    "Si": 71.76,
    "K": 0.39,
    "Ca": 10.56,
    "Ba": 0.0,
    "Fe": 0.18
  },
  {
    "": 130,
    "RI": 1.52177,
    "Na": 13.75,
    "Mg": 1.01,
    "Al": 1.36,
    "Si": 72.19,
    "K": 0.33,
    "Ca": 11.14,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 131,
    "RI": 1.52614,
    "Na": 13.7,
    "Mg": 0.0,
    "Al": 1.36,
    "Si": 71.24,
    "K": 0.19,
    "Ca": 13.44,
    "Ba": 0.0,
    "Fe": 0.1
  },
  {
    "": 132,
    "RI": 1.51813,
    "Na": 13.43,
    "Mg": 3.98,
    "Al": 1.18,
    "Si": 72.49,
    "K": 0.58,
    "Ca": 8.15,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 133,
    "RI": 1.518,
    "Na": 13.71,
    "Mg": 3.93,
    "Al": 1.54,
    "Si": 71.81,
    "K": 0.54,
    "Ca": 8.21,
    "Ba": 0.0,
    "Fe": 0.15
  },
  {
    "": 134,
    "RI": 1.51811,
    "Na": 13.33,
    "Mg": 3.85,
    "Al": 1.25,
    "Si": 72.78,
    "K": 0.52,
    "Ca": 8.12,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 135,
    "RI": 1.51789,
    "Na": 13.19,
    "Mg": 3.9,
    "Al": 1.3,
    "Si": 72.33,
    "K": 0.55,
    "Ca": 8.44,
    "Ba": 0.0,
    "Fe": 0.28
  },
  {
    "": 136,
    "RI": 1.51806,
    "Na": 13.0,
    "Mg": 3.8,
    "Al": 1.08,
    "Si": 73.07,
    "K": 0.56,
    "Ca": 8.38,
    "Ba": 0.0,
    "Fe": 0.12
  },
  {
    "": 137,
    "RI": 1.51711,
    "Na": 12.89,
    "Mg": 3.62,
    "Al": 1.57,
    "Si": 72.96,
    "K": 0.61,
    "Ca": 8.11,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 138,
    "RI": 1.51674,
    "Na": 12.79,
    "Mg": 3.52,
    "Al": 1.54,
    "Si": 73.36,
    "K": 0.66,
    "Ca": 7.9,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 139,
    "RI": 1.51674,
    "Na": 12.87,
    "Mg": 3.56,
    "Al": 1.64,
    "Si": 73.14,
    "K": 0.65,
    "Ca": 7.99,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 140,
    "RI": 1.5169,
    "Na": 13.33,
    "Mg": 3.54,
    "Al": 1.61,
    "Si": 72.54,
    "K": 0.68,
    "Ca": 8.11,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 141,
    "RI": 1.51851,
    "Na": 13.2,
    "Mg": 3.63,
    "Al": 1.07,
    "Si": 72.83,
    "K": 0.57,
    "Ca": 8.41,
    "Ba": 0.09,
    "Fe": 0.17
  },
  {
    "": 142,
    "RI": 1.51662,
    "Na": 12.85,
    "Mg": 3.51,
    "Al": 1.44,
    "Si": 73.01,
    "K": 0.68,
    "Ca": 8.23,
    "Ba": 0.06,
    "Fe": 0.25
  },
  {
    "": 143,
    "RI": 1.51709,
    "Na": 13.0,
    "Mg": 3.47,
    "Al": 1.79,
    "Si": 72.72,
    "K": 0.66,
    "Ca": 8.18,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 144,
    "RI": 1.5166,
    "Na": 12.99,
    "Mg": 3.18,
    "Al": 1.23,
    "Si": 72.97,
    "K": 0.58,
    "Ca": 8.81,
    "Ba": 0.0,
    "Fe": 0.24
  },
  {
    "": 145,
    "RI": 1.51839,
    "Na": 12.85,
    "Mg": 3.67,
    "Al": 1.24,
    "Si": 72.57,
    "K": 0.62,
    "Ca": 8.68,
    "Ba": 0.0,
    "Fe": 0.35
  },
  {
    "": 146,
    "RI": 1.51769,
    "Na": 13.65,
    "Mg": 3.66,
    "Al": 1.11,
    "Si": 72.77,
    "K": 0.11,
    "Ca": 8.6,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 147,
    "RI": 1.5161,
    "Na": 13.33,
    "Mg": 3.53,
    "Al": 1.34,
    "Si": 72.67,
    "K": 0.56,
    "Ca": 8.33,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 148,
    "RI": 1.5167,
    "Na": 13.24,
    "Mg": 3.57,
    "Al": 1.38,
    "Si": 72.7,
    "K": 0.56,
    "Ca": 8.44,
    "Ba": 0.0,
    "Fe": 0.1
  },
  {
    "": 149,
    "RI": 1.51643,
    "Na": 12.16,
    "Mg": 3.52,
    "Al": 1.35,
    "Si": 72.89,
    "K": 0.57,
    "Ca": 8.53,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 150,
    "RI": 1.51665,
    "Na": 13.14,
    "Mg": 3.45,
    "Al": 1.76,
    "Si": 72.48,
    "K": 0.6,
    "Ca": 8.38,
    "Ba": 0.0,
    "Fe": 0.17
  },
  {
    "": 151,
    "RI": 1.52127,
    "Na": 14.32,
    "Mg": 3.9,
    "Al": 0.83,
    "Si": 71.5,
    "K": 0.0,
    "Ca": 9.49,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 152,
    "RI": 1.51779,
    "Na": 13.64,
    "Mg": 3.65,
    "Al": 0.65,
    "Si": 73.0,
    "K": 0.06,
    "Ca": 8.93,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 153,
    "RI": 1.5161,
    "Na": 13.42,
    "Mg": 3.4,
    "Al": 1.22,
    "Si": 72.69,
    "K": 0.59,
    "Ca": 8.32,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 154,
    "RI": 1.51694,
    "Na": 12.86,
    "Mg": 3.58,
    "Al": 1.31,
    "Si": 72.61,
    "K": 0.61,
    "Ca": 8.79,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 155,
    "RI": 1.51646,
    "Na": 13.04,
    "Mg": 3.4,
    "Al": 1.26,
    "Si": 73.01,
    "K": 0.52,
    "Ca": 8.58,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 156,
    "RI": 1.51655,
    "Na": 13.41,
    "Mg": 3.39,
    "Al": 1.28,
    "Si": 72.64,
    "K": 0.52,
    "Ca": 8.65,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 157,
    "RI": 1.52121,
    "Na": 14.03,
    "Mg": 3.76,
    "Al": 0.58,
    "Si": 71.79,
    "K": 0.11,
    "Ca": 9.65,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 158,
    "RI": 1.51776,
    "Na": 13.53,
    "Mg": 3.41,
    "Al": 1.52,
    "Si": 72.04,
    "K": 0.58,
    "Ca": 8.79,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 159,
    "RI": 1.51796,
    "Na": 13.5,
    "Mg": 3.36,
    "Al": 1.63,
    "Si": 71.94,
    "K": 0.57,
    "Ca": 8.81,
    "Ba": 0.0,
    "Fe": 0.09
  },
  {
    "": 160,
    "RI": 1.51832,
    "Na": 13.33,
    "Mg": 3.34,
    "Al": 1.54,
    "Si": 72.14,
    "K": 0.56,
    "Ca": 8.99,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 161,
    "RI": 1.51934,
    "Na": 13.64,
    "Mg": 3.54,
    "Al": 0.75,
    "Si": 72.65,
    "K": 0.16,
    "Ca": 8.89,
    "Ba": 0.15,
    "Fe": 0.24
  },
  {
    "": 162,
    "RI": 1.52211,
    "Na": 14.19,
    "Mg": 3.78,
    "Al": 0.91,
    "Si": 71.36,
    "K": 0.23,
    "Ca": 9.14,
    "Ba": 0.0,
    "Fe": 0.37
  },
  {
    "": 163,
    "RI": 1.51514,
    "Na": 14.01,
    "Mg": 2.68,
    "Al": 3.5,
    "Si": 69.89,
    "K": 1.68,
    "Ca": 5.87,
    "Ba": 2.2,
    "Fe": 0.0
  },
  {
    "": 164,
    "RI": 1.51915,
    "Na": 12.73,
    "Mg": 1.85,
    "Al": 1.86,
    "Si": 72.69,
    "K": 0.6,
    "Ca": 10.09,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 165,
    "RI": 1.52171,
    "Na": 11.56,
    "Mg": 1.88,
    "Al": 1.56,
    "Si": 72.86,
    "K": 0.47,
    "Ca": 11.41,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 166,
    "RI": 1.52151,
    "Na": 11.03,
    "Mg": 1.71,
    "Al": 1.56,
    "Si": 73.44,
    "K": 0.58,
    "Ca": 11.62,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 167,
    "RI": 1.51969,
    "Na": 12.64,
    "Mg": 0.0,
    "Al": 1.65,
    "Si": 73.75,
    "K": 0.38,
    "Ca": 11.53,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 168,
    "RI": 1.51666,
    "Na": 12.86,
    "Mg": 0.0,
    "Al": 1.83,
    "Si": 73.88,
    "K": 0.97,
    "Ca": 10.17,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 169,
    "RI": 1.51994,
    "Na": 13.27,
    "Mg": 0.0,
    "Al": 1.76,
    "Si": 73.03,
    "K": 0.47,
    "Ca": 11.32,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 170,
    "RI": 1.52369,
    "Na": 13.44,
    "Mg": 0.0,
    "Al": 1.58,
    "Si": 72.22,
    "K": 0.32,
    "Ca": 12.24,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 171,
    "RI": 1.51316,
    "Na": 13.02,
    "Mg": 0.0,
    "Al": 3.04,
    "Si": 70.48,
    "K": 6.21,
    "Ca": 6.96,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 172,
    "RI": 1.51321,
    "Na": 13.0,
    "Mg": 0.0,
    "Al": 3.02,
    "Si": 70.7,
    "K": 6.21,
    "Ca": 6.93,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 173,
    "RI": 1.52043,
    "Na": 13.38,
    "Mg": 0.0,
    "Al": 1.4,
    "Si": 72.25,
    "K": 0.33,
    "Ca": 12.5,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 174,
    "RI": 1.52058,
    "Na": 12.85,
    "Mg": 1.61,
    "Al": 2.17,
    "Si": 72.18,
    "K": 0.76,
    "Ca": 9.7,
    "Ba": 0.24,
    "Fe": 0.51
  },
  {
    "": 175,
    "RI": 1.52119,
    "Na": 12.97,
    "Mg": 0.33,
    "Al": 1.51,
    "Si": 73.39,
    "K": 0.13,
    "Ca": 11.27,
    "Ba": 0.0,
    "Fe": 0.28
  },
  {
    "": 176,
    "RI": 1.51905,
    "Na": 14.0,
    "Mg": 2.39,
    "Al": 1.56,
    "Si": 72.37,
    "K": 0.0,
    "Ca": 9.57,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 177,
    "RI": 1.51937,
    "Na": 13.79,
    "Mg": 2.41,
    "Al": 1.19,
    "Si": 72.76,
    "K": 0.0,
    "Ca": 9.77,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 178,
    "RI": 1.51829,
    "Na": 14.46,
    "Mg": 2.24,
    "Al": 1.62,
    "Si": 72.38,
    "K": 0.0,
    "Ca": 9.26,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 179,
    "RI": 1.51852,
    "Na": 14.09,
    "Mg": 2.19,
    "Al": 1.66,
    "Si": 72.67,
    "K": 0.0,
    "Ca": 9.32,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 180,
    "RI": 1.51299,
    "Na": 14.4,
    "Mg": 1.74,
    "Al": 1.54,
    "Si": 74.55,
    "K": 0.0,
    "Ca": 7.59,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 181,
    "RI": 1.51888,
    "Na": 14.99,
    "Mg": 0.78,
    "Al": 1.74,
    "Si": 72.5,
    "K": 0.0,
    "Ca": 9.95,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 182,
    "RI": 1.51916,
    "Na": 14.15,
    "Mg": 0.0,
    "Al": 2.09,
    "Si": 72.74,
    "K": 0.0,
    "Ca": 10.88,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 183,
    "RI": 1.51969,
    "Na": 14.56,
    "Mg": 0.0,
    "Al": 0.56,
    "Si": 73.48,
    "K": 0.0,
    "Ca": 11.22,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 184,
    "RI": 1.51115,
    "Na": 17.38,
    "Mg": 0.0,
    "Al": 0.34,
    "Si": 75.41,
    "K": 0.0,
    "Ca": 6.65,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 185,
    "RI": 1.51131,
    "Na": 13.69,
    "Mg": 3.2,
    "Al": 1.81,
    "Si": 72.81,
    "K": 1.76,
    "Ca": 5.43,
    "Ba": 1.19,
    "Fe": 0.0
  },
  {
    "": 186,
    "RI": 1.51838,
    "Na": 14.32,
    "Mg": 3.26,
    "Al": 2.22,
    "Si": 71.25,
    "K": 1.46,
    "Ca": 5.79,
    "Ba": 1.63,
    "Fe": 0.0
  },
  {
    "": 187,
    "RI": 1.52315,
    "Na": 13.44,
    "Mg": 3.34,
    "Al": 1.23,
    "Si": 72.38,
    "K": 0.6,
    "Ca": 8.83,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 188,
    "RI": 1.52247,
    "Na": 14.86,
    "Mg": 2.2,
    "Al": 2.06,
    "Si": 70.26,
    "K": 0.76,
    "Ca": 9.76,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 189,
    "RI": 1.52365,
    "Na": 15.79,
    "Mg": 1.83,
    "Al": 1.31,
    "Si": 70.43,
    "K": 0.31,
    "Ca": 8.61,
    "Ba": 1.68,
    "Fe": 0.0
  },
  {
    "": 190,
    "RI": 1.51613,
    "Na": 13.88,
    "Mg": 1.78,
    "Al": 1.79,
    "Si": 73.1,
    "K": 0.0,
    "Ca": 8.67,
    "Ba": 0.76,
    "Fe": 0.0
  },
  {
    "": 191,
    "RI": 1.51602,
    "Na": 14.85,
    "Mg": 0.0,
    "Al": 2.38,
    "Si": 73.28,
    "K": 0.0,
    "Ca": 8.76,
    "Ba": 0.64,
    "Fe": 0.09
  },
  {
    "": 192,
    "RI": 1.51623,
    "Na": 14.2,
    "Mg": 0.0,
    "Al": 2.79,
    "Si": 73.46,
    "K": 0.04,
    "Ca": 9.04,
    "Ba": 0.4,
    "Fe": 0.09
  },
  {
    "": 193,
    "RI": 1.51719,
    "Na": 14.75,
    "Mg": 0.0,
    "Al": 2.0,
    "Si": 73.02,
    "K": 0.0,
    "Ca": 8.53,
    "Ba": 1.59,
    "Fe": 0.08
  },
  {
    "": 194,
    "RI": 1.51683,
    "Na": 14.56,
    "Mg": 0.0,
    "Al": 1.98,
    "Si": 73.29,
    "K": 0.0,
    "Ca": 8.52,
    "Ba": 1.57,
    "Fe": 0.07
  },
  {
    "": 195,
    "RI": 1.51545,
    "Na": 14.14,
    "Mg": 0.0,
    "Al": 2.68,
    "Si": 73.39,
    "K": 0.08,
    "Ca": 9.07,
    "Ba": 0.61,
    "Fe": 0.05
  },
  {
    "": 196,
    "RI": 1.51556,
    "Na": 13.87,
    "Mg": 0.0,
    "Al": 2.54,
    "Si": 73.23,
    "K": 0.14,
    "Ca": 9.41,
    "Ba": 0.81,
    "Fe": 0.01
  },
  {
    "": 197,
    "RI": 1.51727,
    "Na": 14.7,
    "Mg": 0.0,
    "Al": 2.34,
    "Si": 73.28,
    "K": 0.0,
    "Ca": 8.95,
    "Ba": 0.66,
    "Fe": 0.0
  },
  {
    "": 198,
    "RI": 1.51531,
    "Na": 14.38,
    "Mg": 0.0,
    "Al": 2.66,
    "Si": 73.1,
    "K": 0.04,
    "Ca": 9.08,
    "Ba": 0.64,
    "Fe": 0.0
  },
  {
    "": 199,
    "RI": 1.51609,
    "Na": 15.01,
    "Mg": 0.0,
    "Al": 2.51,
    "Si": 73.05,
    "K": 0.05,
    "Ca": 8.83,
    "Ba": 0.53,
    "Fe": 0.0
  },
  {
    "": 200,
    "RI": 1.51508,
    "Na": 15.15,
    "Mg": 0.0,
    "Al": 2.25,
    "Si": 73.5,
    "K": 0.0,
    "Ca": 8.34,
    "Ba": 0.63,
    "Fe": 0.0
  },
  {
    "": 201,
    "RI": 1.51653,
    "Na": 11.95,
    "Mg": 0.0,
    "Al": 1.19,
    "Si": 75.18,
    "K": 2.7,
    "Ca": 8.93,
    "Ba": 0.0,
    "Fe": 0.0
  },
  {
    "": 202,
    "RI": 1.51514,
    "Na": 14.85,
    "Mg": 0.0,
    "Al": 2.42,
    "Si": 73.72,
    "K": 0.0,
    "Ca": 8.39,
    "Ba": 0.56,
    "Fe": 0.0
  },
  {
    "": 203,
    "RI": 1.51658,
    "Na": 14.8,
    "Mg": 0.0,
    "Al": 1.99,
    "Si": 73.11,
    "K": 0.0,
    "Ca": 8.28,
    "Ba": 1.71,
    "Fe": 0.0
  },
  {
    "": 204,
    "RI": 1.51617,
    "Na": 14.95,
    "Mg": 0.0,
    "Al": 2.27,
    "Si": 73.3,
    "K": 0.0,
    "Ca": 8.71,
    "Ba": 0.67,
    "Fe": 0.0
  },
  {
    "": 205,
    "RI": 1.51732,
    "Na": 14.95,
    "Mg": 0.0,
    "Al": 1.8,
    "Si": 72.99,
    "K": 0.0,
    "Ca": 8.61,
    "Ba": 1.55,
    "Fe": 0.0
  },
  {
    "": 206,
    "RI": 1.51645,
    "Na": 14.94,
    "Mg": 0.0,
    "Al": 1.87,
    "Si": 73.11,
    "K": 0.0,
    "Ca": 8.67,
    "Ba": 1.38,
    "Fe": 0.0
  },
  {
    "": 207,
    "RI": 1.51831,
    "Na": 14.39,
    "Mg": 0.0,
    "Al": 1.82,
    "Si": 72.86,
    "K": 1.41,
    "Ca": 6.47,
    "Ba": 2.88,
    "Fe": 0.0
  },
  {
    "": 208,
    "RI": 1.5164,
    "Na": 14.37,
    "Mg": 0.0,
    "Al": 2.74,
    "Si": 72.85,
    "K": 0.0,
    "Ca": 9.45,
    "Ba": 0.54,
    "Fe": 0.0
  },
  {
    "": 209,
    "RI": 1.51623,
    "Na": 14.14,
    "Mg": 0.0,
    "Al": 2.88,
    "Si": 72.61,
    "K": 0.08,
    "Ca": 9.18,
    "Ba": 1.06,
    "Fe": 0.0
  },
  {
    "": 210,
    "RI": 1.51685,
    "Na": 14.92,
    "Mg": 0.0,
    "Al": 1.99,
    "Si": 73.06,
    "K": 0.0,
    "Ca": 8.4,
    "Ba": 1.59,
    "Fe": 0.0
  },
  {
    "": 211,
    "RI": 1.52065,
    "Na": 14.36,
    "Mg": 0.0,
    "Al": 2.02,
    "Si": 73.42,
    "K": 0.0,
    "Ca": 8.44,
    "Ba": 1.64,
    "Fe": 0.0
  },
  {
    "": 212,
    "RI": 1.51651,
    "Na": 14.38,
    "Mg": 0.0,
    "Al": 1.94,
    "Si": 73.61,
    "K": 0.0,
    "Ca": 8.48,
    "Ba": 1.57,
    "Fe": 0.0
  },
  {
    "": 213,
    "RI": 1.51711,
    "Na": 14.23,
    "Mg": 0.0,
    "Al": 2.08,
    "Si": 73.36,
    "K": 0.0,
    "Ca": 8.62,
    "Ba": 1.67,
    "Fe": 0.0
  }
]
clasification = [
  {
    "": 0,
    "Type_of_glass": 1
  },
  {
    "": 1,
    "Type_of_glass": 1
  },
  {
    "": 2,
    "Type_of_glass": 1
  },
  {
    "": 3,
    "Type_of_glass": 1
  },
  {
    "": 4,
    "Type_of_glass": 1
  },
  {
    "": 5,
    "Type_of_glass": 1
  },
  {
    "": 6,
    "Type_of_glass": 1
  },
  {
    "": 7,
    "Type_of_glass": 1
  },
  {
    "": 8,
    "Type_of_glass": 1
  },
  {
    "": 9,
    "Type_of_glass": 1
  },
  {
    "": 10,
    "Type_of_glass": 1
  },
  {
    "": 11,
    "Type_of_glass": 1
  },
  {
    "": 12,
    "Type_of_glass": 1
  },
  {
    "": 13,
    "Type_of_glass": 1
  },
  {
    "": 14,
    "Type_of_glass": 1
  },
  {
    "": 15,
    "Type_of_glass": 1
  },
  {
    "": 16,
    "Type_of_glass": 1
  },
  {
    "": 17,
    "Type_of_glass": 1
  },
  {
    "": 18,
    "Type_of_glass": 1
  },
  {
    "": 19,
    "Type_of_glass": 1
  },
  {
    "": 20,
    "Type_of_glass": 1
  },
  {
    "": 21,
    "Type_of_glass": 1
  },
  {
    "": 22,
    "Type_of_glass": 1
  },
  {
    "": 23,
    "Type_of_glass": 1
  },
  {
    "": 24,
    "Type_of_glass": 1
  },
  {
    "": 25,
    "Type_of_glass": 1
  },
  {
    "": 26,
    "Type_of_glass": 1
  },
  {
    "": 27,
    "Type_of_glass": 1
  },
  {
    "": 28,
    "Type_of_glass": 1
  },
  {
    "": 29,
    "Type_of_glass": 1
  },
  {
    "": 30,
    "Type_of_glass": 1
  },
  {
    "": 31,
    "Type_of_glass": 1
  },
  {
    "": 32,
    "Type_of_glass": 1
  },
  {
    "": 33,
    "Type_of_glass": 1
  },
  {
    "": 34,
    "Type_of_glass": 1
  },
  {
    "": 35,
    "Type_of_glass": 1
  },
  {
    "": 36,
    "Type_of_glass": 1
  },
  {
    "": 37,
    "Type_of_glass": 1
  },
  {
    "": 38,
    "Type_of_glass": 1
  },
  {
    "": 39,
    "Type_of_glass": 1
  },
  {
    "": 40,
    "Type_of_glass": 1
  },
  {
    "": 41,
    "Type_of_glass": 1
  },
  {
    "": 42,
    "Type_of_glass": 1
  },
  {
    "": 43,
    "Type_of_glass": 1
  },
  {
    "": 44,
    "Type_of_glass": 1
  },
  {
    "": 45,
    "Type_of_glass": 1
  },
  {
    "": 46,
    "Type_of_glass": 1
  },
  {
    "": 47,
    "Type_of_glass": 1
  },
  {
    "": 48,
    "Type_of_glass": 1
  },
  {
    "": 49,
    "Type_of_glass": 1
  },
  {
    "": 50,
    "Type_of_glass": 1
  },
  {
    "": 51,
    "Type_of_glass": 1
  },
  {
    "": 52,
    "Type_of_glass": 1
  },
  {
    "": 53,
    "Type_of_glass": 1
  },
  {
    "": 54,
    "Type_of_glass": 1
  },
  {
    "": 55,
    "Type_of_glass": 1
  },
  {
    "": 56,
    "Type_of_glass": 1
  },
  {
    "": 57,
    "Type_of_glass": 1
  },
  {
    "": 58,
    "Type_of_glass": 1
  },
  {
    "": 59,
    "Type_of_glass": 1
  },
  {
    "": 60,
    "Type_of_glass": 1
  },
  {
    "": 61,
    "Type_of_glass": 1
  },
  {
    "": 62,
    "Type_of_glass": 1
  },
  {
    "": 63,
    "Type_of_glass": 1
  },
  {
    "": 64,
    "Type_of_glass": 1
  },
  {
    "": 65,
    "Type_of_glass": 1
  },
  {
    "": 66,
    "Type_of_glass": 1
  },
  {
    "": 67,
    "Type_of_glass": 1
  },
  {
    "": 68,
    "Type_of_glass": 1
  },
  {
    "": 69,
    "Type_of_glass": 1
  },
  {
    "": 70,
    "Type_of_glass": 2
  },
  {
    "": 71,
    "Type_of_glass": 2
  },
  {
    "": 72,
    "Type_of_glass": 2
  },
  {
    "": 73,
    "Type_of_glass": 2
  },
  {
    "": 74,
    "Type_of_glass": 2
  },
  {
    "": 75,
    "Type_of_glass": 2
  },
  {
    "": 76,
    "Type_of_glass": 2
  },
  {
    "": 77,
    "Type_of_glass": 2
  },
  {
    "": 78,
    "Type_of_glass": 2
  },
  {
    "": 79,
    "Type_of_glass": 2
  },
  {
    "": 80,
    "Type_of_glass": 2
  },
  {
    "": 81,
    "Type_of_glass": 2
  },
  {
    "": 82,
    "Type_of_glass": 2
  },
  {
    "": 83,
    "Type_of_glass": 2
  },
  {
    "": 84,
    "Type_of_glass": 2
  },
  {
    "": 85,
    "Type_of_glass": 2
  },
  {
    "": 86,
    "Type_of_glass": 2
  },
  {
    "": 87,
    "Type_of_glass": 2
  },
  {
    "": 88,
    "Type_of_glass": 2
  },
  {
    "": 89,
    "Type_of_glass": 2
  },
  {
    "": 90,
    "Type_of_glass": 2
  },
  {
    "": 91,
    "Type_of_glass": 2
  },
  {
    "": 92,
    "Type_of_glass": 2
  },
  {
    "": 93,
    "Type_of_glass": 2
  },
  {
    "": 94,
    "Type_of_glass": 2
  },
  {
    "": 95,
    "Type_of_glass": 2
  },
  {
    "": 96,
    "Type_of_glass": 2
  },
  {
    "": 97,
    "Type_of_glass": 2
  },
  {
    "": 98,
    "Type_of_glass": 2
  },
  {
    "": 99,
    "Type_of_glass": 2
  },
  {
    "": 100,
    "Type_of_glass": 2
  },
  {
    "": 101,
    "Type_of_glass": 2
  },
  {
    "": 102,
    "Type_of_glass": 2
  },
  {
    "": 103,
    "Type_of_glass": 2
  },
  {
    "": 104,
    "Type_of_glass": 2
  },
  {
    "": 105,
    "Type_of_glass": 2
  },
  {
    "": 106,
    "Type_of_glass": 2
  },
  {
    "": 107,
    "Type_of_glass": 2
  },
  {
    "": 108,
    "Type_of_glass": 2
  },
  {
    "": 109,
    "Type_of_glass": 2
  },
  {
    "": 110,
    "Type_of_glass": 2
  },
  {
    "": 111,
    "Type_of_glass": 2
  },
  {
    "": 112,
    "Type_of_glass": 2
  },
  {
    "": 113,
    "Type_of_glass": 2
  },
  {
    "": 114,
    "Type_of_glass": 2
  },
  {
    "": 115,
    "Type_of_glass": 2
  },
  {
    "": 116,
    "Type_of_glass": 2
  },
  {
    "": 117,
    "Type_of_glass": 2
  },
  {
    "": 118,
    "Type_of_glass": 2
  },
  {
    "": 119,
    "Type_of_glass": 2
  },
  {
    "": 120,
    "Type_of_glass": 2
  },
  {
    "": 121,
    "Type_of_glass": 2
  },
  {
    "": 122,
    "Type_of_glass": 2
  },
  {
    "": 123,
    "Type_of_glass": 2
  },
  {
    "": 124,
    "Type_of_glass": 2
  },
  {
    "": 125,
    "Type_of_glass": 2
  },
  {
    "": 126,
    "Type_of_glass": 2
  },
  {
    "": 127,
    "Type_of_glass": 2
  },
  {
    "": 128,
    "Type_of_glass": 2
  },
  {
    "": 129,
    "Type_of_glass": 2
  },
  {
    "": 130,
    "Type_of_glass": 2
  },
  {
    "": 131,
    "Type_of_glass": 2
  },
  {
    "": 132,
    "Type_of_glass": 2
  },
  {
    "": 133,
    "Type_of_glass": 2
  },
  {
    "": 134,
    "Type_of_glass": 2
  },
  {
    "": 135,
    "Type_of_glass": 2
  },
  {
    "": 136,
    "Type_of_glass": 2
  },
  {
    "": 137,
    "Type_of_glass": 2
  },
  {
    "": 138,
    "Type_of_glass": 2
  },
  {
    "": 139,
    "Type_of_glass": 2
  },
  {
    "": 140,
    "Type_of_glass": 2
  },
  {
    "": 141,
    "Type_of_glass": 2
  },
  {
    "": 142,
    "Type_of_glass": 2
  },
  {
    "": 143,
    "Type_of_glass": 2
  },
  {
    "": 144,
    "Type_of_glass": 2
  },
  {
    "": 145,
    "Type_of_glass": 2
  },
  {
    "": 146,
    "Type_of_glass": 3
  },
  {
    "": 147,
    "Type_of_glass": 3
  },
  {
    "": 148,
    "Type_of_glass": 3
  },
  {
    "": 149,
    "Type_of_glass": 3
  },
  {
    "": 150,
    "Type_of_glass": 3
  },
  {
    "": 151,
    "Type_of_glass": 3
  },
  {
    "": 152,
    "Type_of_glass": 3
  },
  {
    "": 153,
    "Type_of_glass": 3
  },
  {
    "": 154,
    "Type_of_glass": 3
  },
  {
    "": 155,
    "Type_of_glass": 3
  },
  {
    "": 156,
    "Type_of_glass": 3
  },
  {
    "": 157,
    "Type_of_glass": 3
  },
  {
    "": 158,
    "Type_of_glass": 3
  },
  {
    "": 159,
    "Type_of_glass": 3
  },
  {
    "": 160,
    "Type_of_glass": 3
  },
  {
    "": 161,
    "Type_of_glass": 3
  },
  {
    "": 162,
    "Type_of_glass": 3
  },
  {
    "": 163,
    "Type_of_glass": 5
  },
  {
    "": 164,
    "Type_of_glass": 5
  },
  {
    "": 165,
    "Type_of_glass": 5
  },
  {
    "": 166,
    "Type_of_glass": 5
  },
  {
    "": 167,
    "Type_of_glass": 5
  },
  {
    "": 168,
    "Type_of_glass": 5
  },
  {
    "": 169,
    "Type_of_glass": 5
  },
  {
    "": 170,
    "Type_of_glass": 5
  },
  {
    "": 171,
    "Type_of_glass": 5
  },
  {
    "": 172,
    "Type_of_glass": 5
  },
  {
    "": 173,
    "Type_of_glass": 5
  },
  {
    "": 174,
    "Type_of_glass": 5
  },
  {
    "": 175,
    "Type_of_glass": 5
  },
  {
    "": 176,
    "Type_of_glass": 6
  },
  {
    "": 177,
    "Type_of_glass": 6
  },
  {
    "": 178,
    "Type_of_glass": 6
  },
  {
    "": 179,
    "Type_of_glass": 6
  },
  {
    "": 180,
    "Type_of_glass": 6
  },
  {
    "": 181,
    "Type_of_glass": 6
  },
  {
    "": 182,
    "Type_of_glass": 6
  },
  {
    "": 183,
    "Type_of_glass": 6
  },
  {
    "": 184,
    "Type_of_glass": 6
  },
  {
    "": 185,
    "Type_of_glass": 7
  },
  {
    "": 186,
    "Type_of_glass": 7
  },
  {
    "": 187,
    "Type_of_glass": 7
  },
  {
    "": 188,
    "Type_of_glass": 7
  },
  {
    "": 189,
    "Type_of_glass": 7
  },
  {
    "": 190,
    "Type_of_glass": 7
  },
  {
    "": 191,
    "Type_of_glass": 7
  },
  {
    "": 192,
    "Type_of_glass": 7
  },
  {
    "": 193,
    "Type_of_glass": 7
  },
  {
    "": 194,
    "Type_of_glass": 7
  },
  {
    "": 195,
    "Type_of_glass": 7
  },
  {
    "": 196,
    "Type_of_glass": 7
  },
  {
    "": 197,
    "Type_of_glass": 7
  },
  {
    "": 198,
    "Type_of_glass": 7
  },
  {
    "": 199,
    "Type_of_glass": 7
  },
  {
    "": 200,
    "Type_of_glass": 7
  },
  {
    "": 201,
    "Type_of_glass": 7
  },
  {
    "": 202,
    "Type_of_glass": 7
  },
  {
    "": 203,
    "Type_of_glass": 7
  },
  {
    "": 204,
    "Type_of_glass": 7
  },
  {
    "": 205,
    "Type_of_glass": 7
  },
  {
    "": 206,
    "Type_of_glass": 7
  },
  {
    "": 207,
    "Type_of_glass": 7
  },
  {
    "": 208,
    "Type_of_glass": 7
  },
  {
    "": 209,
    "Type_of_glass": 7
  },
  {
    "": 210,
    "Type_of_glass": 7
  },
  {
    "": 211,
    "Type_of_glass": 7
  },
  {
    "": 212,
    "Type_of_glass": 7
  },
  {
    "": 213,
    "Type_of_glass": 7
  }
]

training_ids = []
testing_ids = []

training_data = []
training_clasifications = []

testing_data = []
testing_clasifications = []

def testing_training_split(min_id, max_id, training_split):
    for i in range(min_id, max_id):
        if random.randint(0,100) <= training_split:
            training_ids.append(i)
        else:
            testing_ids.append(i)


def split_data(id):
    temp = []
    temp1 = []
    holder = features[id]
    temp.append(holder["RI"])
    temp.append(holder["Na"])
    temp.append(holder["Mg"])
    temp.append(holder["Al"])
    temp.append(holder["Si"])
    temp.append(holder["K"])
    temp.append(holder["Ca"])
    temp.append(holder["Ba"])
    temp.append(holder["Fe"])
    temp1.append(clasification[id]["Type_of_glass"]/10)
    return temp, temp1

def make_data_arrays(training, testing):
    for id in training:
        temp1, temp2 = split_data(id)
        training_data.append(temp1)
        training_clasifications.append(temp2)

    for id in testing:
        temp1, temp2 = split_data(id)
        testing_data.append(temp1)
        testing_clasifications.append(temp2)
    
def main():
    testing_training_split(0, 213, 70)
    make_data_arrays(training_ids, testing_ids)

    topology = [9,5,5,1]
    net = Network(topology)
    net.output_network()
    Neuron.eta = 0.09
    Neuron.alpha = 0.015
    inputs = [[0, 0], [0, 1], [1, 0], [1, 1]]
    outputs = [[0, 0], [1, 0], [1, 0], [0, 1]]
    i=0


    #while i <=100:
    #    err = 0
    #    for i in range(len(inputs)):
    #        net.setInput(inputs[i])
    #        net.feedForword()
    #        net.backPropagate(outputs[i])
    #        err = err + net.getError(outputs[i])
    #    print( "error: ", err)
    #    if err < 0.01:
    #        break
    #    i = i +1

    counter = 0
    while counter < len(training_clasifications):
        err = 0
        for i in range(len(training_data)):
            net.setInput(training_data[i])
            net.feedForword()
            net.backPropagate(training_clasifications[i])
            err = err + net.getError(training_clasifications[i])
            #uncoment to see error change durring training
            #print( "error: ", err)
        counter = counter + 1

    #while True:
    #    a = input("type 1st input :")
    #    b = input("type 2nd input :")
    #    net.setInput([a, b])
    #    net.feedForword()
    #    print(net.getThResults())

    net.setInput(testing_data[len(testing_clasifications)-1])
    net.feedForword()
    #print(net.getThResults())
    #print(testing_clasifications)


if __name__ == '__main__':
    main()











